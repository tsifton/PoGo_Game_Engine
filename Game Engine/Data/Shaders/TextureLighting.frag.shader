...
void main()
{
	...
	{
		// Vector that goes from the fragment to the light, in world space.
		...
		vec3 MaterialAmbientColor = uAmbientLightColor * MaterialColor;
		...
		vec3 MaterialDiffuseColor = MaterialColor * uDiffuseLightColor * cosTheta * attenuation;
		...
		vec3 MaterialSpecularColor = uSpecularLightColor * spec * MaterialColor * attenuation;

		// Combine the Material's Ambient, Diffuse Color, and Specular Color
		color += vec4(MaterialAmbientColor + MaterialDiffuseColor + MaterialSpecularColor, 1);
	}
