...
layout(location = 2) noperspective in vec3 fEdgeDistance;
...
vec3 CalculatePhongReflection(Light light, vec4 fragPosition_cameraspace, vec4 fragNormal_cameraspace)
{
	...
	vec3 diffuseColor = light.diffuse * cosTheta * material.diffuse;
...
void main()
{
	...
	float dist = min(min(fEdgeDistance.x, fEdgeDistance.y), fEdgeDistance.z);
	float mixVal = smoothstep(line.width - 1, line.width + 1, dist);
	fragColor = vec4(uEmissiveColor, 1) + accumulativeColor;
	fragColor = mix(line.color, fragColor, mixVal);
}
