...
vec3 CalculatePhongReflection(vec4 vPosition_cameraspace, vec4 normalNormEye)
{
	...
	vec4 LightPosition_cameraspace = uViewMat * uLightPosition;

	vec4 SurfaceToLight_cameraspace;
	if (uLightPosition.w == 1.0f)
		SurfaceToLight_cameraspace = LightPosition_cameraspace - vPosition_cameraspace;
	else
		SurfaceToLight_cameraspace = LightPosition_cameraspace - (uViewMat * uObjectPositon);
