...
subroutine vec3 shaderModelType(vec4 vPosition_cameraspace, vec4 normalNormEye);
...
vec3 PhongModel(vec4 vPosition_cameraspace, vec4 normalNormEye)
{
...
subroutine(shaderModelType)
vec3 DiffuseOnly(vec4 vPosition_cameraspace, vec4 normalNormEye)
{
	// Vector that goes from the fragment to the light, in camera space.
	vec4 LightPosition_cameraspace = uViewMat * vec4(uLightPosition, 1);
	vec4 SurfaceToLight_cameraspace = LightPosition_cameraspace - vPosition_cameraspace;
	...
	// Calculate the Material's Diffuse Color
	return uMaterialColor * uDiffuseLightColor * cosTheta;
}
...
void main()
{
	...
	color = clamp(vec4(uEmissiveColor + shaderModel(vPosition_cameraspace, vNormalizedNormal_cameraspace), 1), 0, 1);
