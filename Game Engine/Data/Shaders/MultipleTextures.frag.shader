...
layout(binding = 1) uniform sampler2D mossyTexture;
...
vec3 CalculatePhongReflection(Light light, vec4 fragPosition_cameraspace, vec4 fragNormal_cameraspace)
{
	...
	vec3 ambientColor = light.ambient * textureColor.rgb;
	...
	vec3 diffuseColor = light.diffuse * cosTheta * textureColor.rgb;
